...
Для доступа к защищенным эндпоинтам (таким как `GET /users/me`, `POST /workouts/generate` и др.) все запросы должны быть аутентифицированы. Система поддерживает два способа аутентификации: по **JWT токену** (для фронтенда) и по **ID пользователя Telegram** (для ботов).

### Способ 1: Для фронтенда (JWT Токен)

Этот способ является стандартным для веб-приложений.

#### Шаг 1: Получение токена

Для получения токена необходимо отправить `POST` запрос на эндпоинт `/auth/login` с `username` и `password` в теле запроса (в формате `x-www-form-urlencoded`). В ответ вы получите `access_token`.
...
Для аутентификации на защищенных эндпоинтах необходимо передавать полученный токен в заголовке `Authorization`.
...
### Способ 2: Для ботов (Telegram ID)

Этот способ предназначен для серверных интеграций, в частности для Telegram-бота, которому небезопасно хранить JWT.

#### Использование

Для аутентификации от имени пользователя необходимо в каждый запрос к защищенному эндпоинту добавлять специальный заголовок `X-Telegram-User-ID`, в котором передается уникальный идентификатор пользователя из Telegram.
...
Сервер получит этот ID, найдет соответствующего пользователя в базе данных и выполнит запрос от его имени.

**Важно:** Если в запросе одновременно присутствуют заголовок `Authorization` с JWT и заголовок `X-Telegram-User-ID`, приоритет будет отдан аутентификации по **`X-Telegram-User-ID`**.
